...
var dmskmidrange = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( dmskmidrange instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dmskmidrange, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( dmskmidrange.length, 7, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the mid-range of a strided array according to a mask', opts, function test( t ) {
	var mask;
	var x;
	var v;
	...
	v = dmskmidrange( x.length, x, 1, 0, mask, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dmskmidrange( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dmskmidrange( -1, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports `stride` parameters', opts, function test( t ) {
	...
	v = dmskmidrange( 5, x, 2, 0, mask, 2, 0 );
...
tape( 'the function supports negative `stride` parameters', opts, function test( t ) {
	...
	v = dmskmidrange( 5, x, -2, 8, mask, -2, 8 );
...
tape( 'the function supports `offset` parameters', opts, function test( t ) {
	...
	v = dmskmidrange( 5, x, 2, 1, mask, 2, 1 );
	t.strictEqual( v, 1.0, 'returns expected value' );
